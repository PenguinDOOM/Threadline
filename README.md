...
| `--upstream-connect-timeout-secs` | `THREADLINE_UPSTREAM_CONNECT_TIMEOUT_SECS` | `30` | Maximum total time to establish an upstream websocket connection. Accepted range: integer `1..=3600` seconds. |
| `--upstream-pong-timeout-secs` | `THREADLINE_UPSTREAM_PONG_TIMEOUT_SECS` | `60` | Maximum time to wait for the matching upstream websocket Pong after a liveness challenge. Accepted range: integer `1..=3600` seconds. |
| `--upstream-write-timeout-secs` | `THREADLINE_UPSTREAM_WRITE_TIMEOUT_SECS` | `60` | Maximum total time for one upstream websocket Text, Ping, or automatic control-frame flush operation. Accepted range: integer `1..=3600` seconds. |
...
Upstream connection establishment has a finite timeout. `--upstream-connect-timeout-secs` and `THREADLINE_UPSTREAM_CONNECT_TIMEOUT_SECS` accept integer values from `1` through `3600`, default to `30`, and use the CLI value when both CLI and environment values are provided. Zero, negative, non-integer, and out-of-range values are rejected during startup; the setting cannot be disabled or clamped. The timeout covers the total asynchronous DNS, TCP, TLS, and HTTP upgrade wait. It does not time out synchronous authentication discovery or refresh work, and it does not guarantee that a blocking internal operation stops physically at the same instant.

The same connection policy applies to Main, Utility, and auxiliary summary upstream connections. When the deadline expires, Threadline drops the connection future, does not create a pump, and does not retry automatically. Before response headers, the existing JSON error is HTTP `502` with `type=server_error`, code `upstream_websocket_connect_timeout`, and message `The upstream websocket connection timed out.`. Existing handshake rejection and other connection errors keep their existing classifications. This is an operational default, not a measured Codex service-level agreement.
...
## Upstream WebSocket Liveness

Threadline also monitors an established upstream WebSocket with immutable Pong and write deadlines. `--upstream-pong-timeout-secs` / `THREADLINE_UPSTREAM_PONG_TIMEOUT_SECS` and `--upstream-write-timeout-secs` / `THREADLINE_UPSTREAM_WRITE_TIMEOUT_SECS` both default to `60` seconds and accept only integer values from `1` through `3600`. CLI values take precedence over environment values. Zero, negative, non-integer, and out-of-range values are rejected at startup; neither timeout can be disabled or clamped. The same policy applies to Main, Utility, auxiliary summary, and retained-session reconnect connections. The existing Ping interval is 30 seconds, with the first Ping sent after 30 seconds; it is not a public setting.

The pump sends one liveness Ping challenge at a time and accepts only a Pong with the current challenge payload. The Pong deadline starts when that Ping send completes. A matching Pong observed during the send is accepted, but ordinary data, server Ping frames, mismatched, stale, or unsolicited Pong frames do not extend the deadline. The pump continues reading while a Text send, client Ping, or automatic control-frame flush is pending, but each such operation has its own fixed total write deadline. A healthy matching Pong therefore does not prevent a write timeout when the send or flush itself remains slow.

This is transport liveness, not an assistant-output deadline. Threadline adds no assistant, event, or output idle timeout: long reasoning, internal-tool work, jobs, and an otherwise idle retained session remain available while the WebSocket is healthy. The `60`-second defaults are operational policy values, not measured Codex service-level agreements.

When either deadline expires, Threadline drops the socket and records one sticky liveness timeout. Before response headers, the public error is HTTP `502` with `type=server_error`, code `upstream_liveness_timeout`, and message `The upstream websocket liveness check timed out.`. After SSE has begun, the existing terminal form emits one `response.failed` with that code and message followed by one `[DONE]`; it adds no new SSE error type. Diagnostic logging records the deadline cause, configured timeout, elapsed time, and outbound operation kind without recording payloads, tool arguments, challenge data, or secrets.

Application control state is limited to one client challenge and whether an automatic control-frame flush is needed. Threadline relies on tungstenite for server-Ping Pong responses and does not set a transport `max_write_buffer_size`; its internal encoded write buffer therefore has no fixed byte bound under a stalled peer. The write deadline and socket drop bound how long that accumulation can persist, but do not establish a fixed byte limit or change existing Text outbound capacity behavior.

For Main continuations, an idle retained session that has already completed successfully and later reaches this timeout is lazily detached while its older marker aliases and metadata remain. A later continuation receives `previous_response_not_found`; Threadline does not reconnect or replay it. A timeout before a continuation's first upstream event invalidates the armed unfinished aliases and returns the same stale-marker signal. A new Main turn, or a timeout after upstream data has been observed, returns the liveness error. In the latter case, previously accepted markers remain recoverable, but a markerless entry is removed. Utility and auxiliary summary routes only release their transient connection and never alter Main retained markers. A final completion accepted before a later timeout remains successful; queued output or completion that has not crossed the final acceptance boundary is rejected. Started tools or sends are not rolled back, and a timeout observed while tool work or downstream polling is paused can become visible only when that flow resumes.
